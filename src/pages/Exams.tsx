...
import { supabase, type Exam, type Class } from '../lib/supabase';
...
export default function Exams() {
  ...
  async function fetchAll() {
    ...
    const [eRes, cRes] = await Promise.all([
      supabase.from('exams').select('*, classes(name)').order('date', { ascending: true }),
      supabase.from('classes').select('*').order('grade'),
    ]);
    setExams((eRes.data || []) as unknown as Exam[]);
  ...
  async function handleSave() {
    ...
    await supabase.from('exams').insert({
      ...
      date: form.date, max_marks: parseInt(form.max_marks) || 100,
      ...
      <div className={`bg-white rounded-2xl border shadow-sm p-4 hover:shadow-md transition-shadow ${isPast ? 'border-gray-100 opacity-75' : 'border-sky-100'}`}>
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => handleDelete(exam.id)} className="p-1 text-gray-300 hover:text-rose-500 transition-colors"><X size={14} /></button>
          </div>
        </div>
        <h4 className="font-semibold text-gray-900 text-sm leading-tight">{exam.name}</h4>
        <p className="text-xs text-sky-600 font-medium mt-0.5">{exam.subject}</p>
        <p className="text-xs text-gray-400 mt-1">{(exam as any).classes?.name || 'All Classes'}</p>
        <div className="mt-3 pt-3 border-t border-gray-50 flex items-center justify-between text-xs text-gray-500">
          ...
          <span>{exam.max_marks} marks • {exam.duration_minutes}min</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex justify-end">
        <button onClick={() => { setForm({ name: '', class_id: '', subject: '', date: '', max_marks: '100', duration_minutes: '60' }); setShowModal(true); }} className="flex items-center gap-2 bg-sky-500 hover:bg-sky-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
      ...
      {loading ? <LoadingSpinner /> : exams.length === 0 ? <EmptyState message="No exams scheduled" icon={BookOpen} /> : (
        <>
          {upcoming.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-gray-600 mb-3">Upcoming Exams ({upcoming.length})</h3>
          ...
          {past.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-gray-400 mb-3 mt-4">Completed Exams ({past.length})</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-3">
              <div>
                ...
                <input value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-200" placeholder="Mid-Term Mathematics" />
              </div>
              ...
              <div>
                ...
                <input type="date" value={form.date} onChange={e => setForm({...form, date: e.target.value})} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-200" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-1">Max Marks</label>
                  <input type="number" value={form.max_marks} onChange={e => setForm({...form, max_marks: e.target.value})} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-200" />
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-600 block mb-1">Duration (min)</label>
                  <input type="number" value={form.duration_minutes} onChange={e => setForm({...form, duration_minutes: e.target.value})} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-200" />
                </div>
              </div>
            </div>
